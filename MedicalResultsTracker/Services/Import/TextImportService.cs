...
namespace MedicalResultsTracker.Services.Import
{
    ...
    public sealed class TextImportService : ITextImportService
    {
        ...
        /// <summary>
        /// Запрос лежит в ресурсах, а не в коде: человек его читает и вставляет в свой чат,
        /// то есть это такой же текст интерфейса, как надписи на кнопках. Разбор ответа
        /// понимает шапку и на немецком, и на русском, поэтому переведённый запрос
        /// возвращается ровно тем же форматом.
        /// </summary>
        public string BuildPrompt(IReadOnlyList<Analyte> known)
        {
            ...
            if (names.Length == 0)
            {
                return S.Imp_Prompt + Environment.NewLine + S.Imp_PromptFreeNames;
            }

            // Список идёт после правил: сначала формат, потом справочные данные к нему.
            return S.Imp_Prompt + Environment.NewLine + S.Imp_PromptKnownNames +
